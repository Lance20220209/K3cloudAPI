...
{
    [Description("委外领料单审核生成胜途的其他出库单")]
    [Kingdee.BOS.Util.HotUpdate]
    public class ZWLF_SUB_PickMtrlAudit : AbstractOperationServicePlugIn
    {
            ...
            if (Context.DBId == zWLF_Configuration.DBId)
            {
                if (e.DataEntitys != null && e.DataEntitys.Count<DynamicObject>() > 0)
                {
                    foreach (DynamicObject item in e.DataEntitys)
                    {
                        ...
                        //委外领料表
                        sql += string.Format(@"/*dialect*/ select d.FBILLNO,a.FSTOCKID, d.F_ZWLF_PUSHSTATE,b.FNUMBER as wlFNUMBER,FACTUALQTY,d.FDATE 
                                                        from  T_SUB_PICKMTRLDATA  a
                                                        inner join T_BD_MATERIAL b on a.FMATERIALID=b.FMATERIALID
                                                        inner  join T_SUB_PICKMTRL d on d.FID=a.FID
        ...
        public bool STOtheroutIn(Parameters param, DataSet ds)
        {
            ...
            try
            {
                ...
                //备注 
                @params["memo_info"] = "金蝶的委外领料单生成胜途的其他出库单";
                //参与库龄分析 
                ...
                for (int i = 0; i < dt_c.Rows.Count; i++)
                {
                    ...
                    jObject.Add("qty", Convert.ToInt32(dt_c.Rows[i]["FACTUALQTY"]));
                ...
                if (code == "200")
                {
                    ...
                    upsql = string.Format(@"/*dialect*/ update T_SUB_PICKMTRL set F_ZWLF_PUSHSTATE=1,F_ZWLF_PUSHTIME=GETDATE(),
