...
{
    [Description("其他入库单审核生成胜途的其他入库单")]
    [Kingdee.BOS.Util.HotUpdate]
    public class ZWLF_STK_MISCELLANEOUS : AbstractOperationServicePlugIn
    {
                ...
                if (e.DataEntitys != null && e.DataEntitys.Count<DynamicObject>() > 0)
                {
                    foreach (DynamicObject item in e.DataEntitys)
                    {
                        ...
                        sql += string.Format(@"/*dialect*/ select d.FBILLNO, d.F_ZWLF_PUSHSTATE,t.FNUMBER as wh_code,a.FSTOCKID,b.FNUMBER as wlFNUMBER, FQTY ,d.FDATE 
                                                           ...
                                                           inner  join T_STK_MISCELLANEOUS d on d.FID=a.FID
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            {
                                ZWLF_STK_MisDeliveryAudit zWLF_STK_MisDeliveryAudit = new ZWLF_STK_MisDeliveryAudit();
                                bool retult = zWLF_STK_MisDeliveryAudit.STOtheroutIn(parameters, dt_t,"2");
                            }
                        }
                    }
                }
        ...
        /// <returns></returns>
        public bool STOtheroutIn(Parameters param, DataTable dt,string type)
        {
            bool retult = true;
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        //备注 
                        @params["memo_info"] = "金蝶的其他入库单生成胜途的其他入库单";
                    }
                    else
                    {
                        //备注 
                        @params["memo_info"] = "金蝶的其他出库退货生成胜途的其他入库单";
                    }
                    //参与库龄分析 
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        jObject.Add("prdt_code", dt.Rows[i]["wlFNUMBER"].ToString());
                    ...
                    if (code == "200")
                    {
                        ...
                        upsql = string.Format(@"/*dialect*/ update T_STK_MISCELLANEOUS set F_ZWLF_PUSHSTATE=1,F_ZWLF_PUSHTIME=GETDATE(),
