...
{
    [Description("生产领料审核生成胜途的其他出库单")]
    [Kingdee.BOS.Util.HotUpdate]
    public class ZWLF_PRD_PickMtrl : AbstractOperationServicePlugIn
    {
            ...
            if (Context.DBId == zWLF_Configuration.DBId)
            {
                if (e.DataEntitys != null && e.DataEntitys.Count<DynamicObject>() > 0)
                {
                    foreach (DynamicObject item in e.DataEntitys)
                    {
                        ...
                        //生产领料表
                        sql += string.Format(@"/*dialect*/ select a.FSTOCKID ,d.FBILLNO, d.F_ZWLF_PUSHSTATE,b.FNUMBER as wlFNUMBER, FACTUALQTY ,d.FDATE 
                                                        from  T_PRD_PICKMTRLDATA  a
                                                        inner join T_BD_MATERIAL b on a.FMATERIALID=b.FMATERIALID
                                                        inner  join T_PRD_PICKMTRL d on d.FID=a.FID
        ...
        public bool STOtheroutIn(Parameters param, DataSet ds)
        {
            ...
            try
            {
                ...
                //备注 
                @params["memo_info"] = "金蝶的生产领料单生成胜途的其他出库单";
                //参与库龄分析 
                ...
                if (code == "200")
                {
                    ...
                    upsql = string.Format(@"/*dialect*/ update T_PRD_PICKMTRL set F_ZWLF_PUSHSTATE=1,F_ZWLF_PUSHTIME=GETDATE(),
