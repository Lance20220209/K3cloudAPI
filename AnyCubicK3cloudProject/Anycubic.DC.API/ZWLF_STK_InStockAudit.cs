...
{
    [Description("采购入库单审核推送胜途")]
    [Kingdee.BOS.Util.HotUpdate]
    public class ZWLF_STK_InStockAudit : AbstractOperationServicePlugIn
    {
            ...
            if (Context.DBId == zWLF_Configuration.DBId)
            {
                if (e.DataEntitys != null && e.DataEntitys.Count<DynamicObject>() > 0)
                {
                    foreach (DynamicObject item in e.DataEntitys)
                    {
                        ...
                        //对应胜途采购入库单
                        sql += string.Format(@"/*dialect*/select  FBILLNO,FDATE, FSTOCKORGID,b.FSTOCKID as FSTOCKID,a.FAPPROVEDATE ,wl.FNUMBER, FREALQTY as FQTY, b.FNOTE  from  t_STK_InStock a 
                                                         join T_STK_INSTOCKENTRY b on a.FID=b.FID
                                                         join T_BD_MATERIAL wl on wl.FMATERIALID=b.FMATERIALID
					                                     join t_STK_InStockFin c on c.FID=a.FID
                                                         where  a.F_ZWLF_PUSHSTATE!='1' and  c.FISGENFORIOS=0 and  a.FID={0} 
        ...
        public bool STOtherIn(Parameters param, DataTable dt)
        {
            ...
            try
            {
                ...
                //备注 
                @params["memo_info"] = "金蝶的采购入库单：" + dt.Rows[0]["FBILLNO"].ToString() + "单生成胜途的采购入库单";
                //参与库龄分析 
                ...
                if (code == "200")
                {
                    ...
                    upsql = string.Format(@"/*dialect*/ update t_STK_InStock set F_ZWLF_PUSHSTATE=1,F_ZWLF_PUSHTIME=GETDATE(),
