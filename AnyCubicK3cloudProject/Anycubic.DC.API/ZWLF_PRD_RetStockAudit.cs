...
{
    [Description("生产退库审核生成胜途的其他出库单")]
    [Kingdee.BOS.Util.HotUpdate]
    public class ZWLF_PRD_RetStockAudit : AbstractOperationServicePlugIn
    {
            ...
            if (Context.DBId == zWLF_Configuration.DBId)
            {
                if (e.DataEntitys != null && e.DataEntitys.Count<DynamicObject>() > 0)
                {
                    foreach (DynamicObject item in e.DataEntitys)
                    {
                        ...
                        //生产退库
                        sql += string.Format(@"/*dialect*/ select a.FSTOCKID ,d.FBILLNO, d.F_ZWLF_PUSHSTATE,b.FNUMBER as wlFNUMBER, FREALQTY ,d.FDATE 
                                                        from  T_PRD_RESTOCKENTRY   a
                                                        inner join T_BD_MATERIAL b on a.FMATERIALID=b.FMATERIALID
                                                        inner  join T_PRD_RESTOCK d on d.FID=a.FID
                                                        where  a.FSTOCKID in (171002,103583,226441,171005,270325,102502,450697,450698)
                                                        and FISGENFORIOS=0 and d.FID={0}", Id);
        ...
        public bool STOtheroutIn(Parameters param, DataSet ds)
        {
            ...
            try
            {
                ...
                //备注 
                @params["memo_info"] = "金蝶的生产退库单生成胜途的其他出库单";
                //参与库龄分析 
                ...
                for (int i = 0; i < dt_c.Rows.Count; i++)
                {
                    ...
                    jObject.Add("qty", Convert.ToInt32(dt_c.Rows[i]["FREALQTY"]));
                ...
                if (code == "200")
                {
                    ...
                    upsql = string.Format(@"/*dialect*/ update T_PRD_RESTOCK set F_ZWLF_PUSHSTATE=1,F_ZWLF_PUSHTIME=GETDATE(),
