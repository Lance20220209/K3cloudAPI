        ...
        public void Run(Kingdee.BOS.Context ctx, Schedule schedule)
        {
            try
            {
                ...
                string sql = string.Format(@"/*dialect*/ select  app_key,AppSecret,date_type,end_date,page_no,page_size,
                                    ...
                                    from ZWLF_T_Configuration where id=1");
                sql += string.Format(@"/*dialect*/ select  top 100 id ,method  from ZWLF_T_SaleOut  where IsPush =10  and  site_type in ('Amazon','Walmart') order  by send_date asc ");
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        DateTime Btime = Convert.ToDateTime(dt.Rows[i]["start_date"].ToString()).AddMonths(-5);
                    ...
                    if (site_type == "Amazon"|| site_type== "Walmart")
                    {
                        ...
                        if (FStockOrgId == "165223")
                        {
                            ...
                            if (OrderRow.Length == 0)
                            {
                                ...
                                DateTime dt1= Convert.ToDateTime(Convert.ToDateTime(array[i]["send_date"].ToString()).ToString("yyyy-MM-dd"));
