...
{
    [Description("获取胜途所有调拨入库单保存至中间表")]
    [Kingdee.BOS.Util.HotUpdate]
    public class ZWLF_GetTranferInToMiddle : IScheduleService
    {
        /// <summary>
        /// 定时任务
        /// </summary>
        ...
        public void Run(Kingdee.BOS.Context ctx, Schedule schedule)
        {
            try
            {
                ...
                string sql = string.Format(@"/*dialect*/ select top 1  app_key,AppSecret,date_type,end_date,page_no,page_size,
                                    ...
                                    from ZWLF_T_Configuration where id=33 and IsDisable=0");
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        msg = GetTranferInToMiddle(parameters, context);
                        if (msg.status)
                        {
        ...
        /// <returns></returns>
        public Msg GetTranferInToMiddle(Parameters param, Context context)
        {
            Msg msg = new Msg();
            try
            {
                ...
                if (code == "200")
                {
                    ...
                    if (total_count > 0)
                    {
                        ...
                        for (int i = 0; i < array.Count; i++)
                        {
                            ...
                            string dept_top_out = "";
                            if (array[i].ToString().Contains("dept_top_out"))
                            {
                                dept_top_out = array[i]["dept_top_out"].ToString();
                            }
                            ...
                            string user_name = "";
                            if (array[i].ToString().Contains("user_name"))
                            {
                                user_name = array[i]["user_name"].ToString();
                            }
                            string chk_date_in = "";
                            if (array[i].ToString().Contains("chk_date_in"))
                            {
                                chk_date_in = array[i]["chk_date_in"].ToString();
                            }
                            string dept_top_in = "";
                            if (array[i].ToString().Contains("dept_top_in"))
                            {
                                dept_top_in = array[i]["dept_top_in"].ToString();
                            }
                            //string chk_user_out = array[i]["chk_user_out"].ToString();
                            string dept_top_create = "";
                            if (array[i].ToString().Contains("dept_top_create"))
                            {
                                dept_top_create = array[i]["dept_top_create"].ToString();
                            }
                            string modified = "";
                            if (array[i].ToString().Contains("modified"))
                            {
                                 modified = array[i]["modified"].ToString();
                            }
                                //表头
                                sql += string.Format(@" /*dialect*/ if not exists (select  *  from ZWLF_T_TranferIn where id ='{0}') begin INSERT INTO ZWLF_T_TranferIn
                                                 ([id]
                                                 ,[chk_user]
                                                 ,[created]
                                                 ,[wh_code_in]
                                           ...
                                           VALUES
                                                 ...
                                                 ,'{14}'
                                                 ,'{15}'
                                                 ,'{16}' ) end ;", id, chk_user, created, wh_code_in, dept_top_in, expected_date
                                                 , wh_code_out, bil_date, chk_date_in, user_name, wh_name_in
                                                 , wh_name_out, dept_top_out, bil_no, chk_user_name, dept_top_create, modified);
                                //表体
                                ...
                                for (int j = 0; j < details.Count; j++)
                                {
                                    ...
                                    int qty_in = Convert.ToInt32(details[j]["qty_in"].ToString());
                                    string Fmemo_info = "";
                                    ...
                                    sql += string.Format(@"/*dialect*/ if not exists (select  *  from ZWLF_T_TranferInEntry where id ='{0}') begin INSERT INTO ZWLF_T_TranferInEntry
                                                       ([id]
                                                       ,[parent_id]
                                                       ,[prdt_code]
                                                       ,[prdt_name]
                                                 ...
                                                 VALUES
                                                       ...
                                                       ,'{8}') end ;", Fid, parent_id, prdt_code, prdt_name,
                                                             spc, memo_info, qty_in, unit_name, prdt_id);
                ...
                else
                {
                    //记录每一页获取情况
                    msg.status = false;
                    msg.result = "获取所有调入出库单报错：" + OrderJson.ToString().Substring(0, 500).Replace("'", "");
