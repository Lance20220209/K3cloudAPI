...
namespace AnyCubicK3cloudProject
{
    ...
    [Description("重新获取失败的调拨入库单")]
    [Kingdee.BOS.Util.HotUpdate]
    public  class ZWLF_GetFailTransferIn : AbstractListPlugIn
    {
        public override void BarItemClick(Kingdee.BOS.Core.DynamicForm.PlugIn.Args.BarItemClickEventArgs e)
        {
            base.BarItemClick(e);
            if (e.BarItemKey.Equals("ZWLF_tbGetFailTransferIn"))
            {
                string mssg = "";
                ...
                if (listcoll.Count > 0)
                {
                    ...
                    ZWLF_GetTransferIn zWLF_GetTransferIn = new ZWLF_GetTransferIn();
                    ...
                    sql += string.Format(@"/*dialect*/ select distinct  F_ZWLF_ID ,F_ZWLF_IsOverseas from  ZWLF_T_OrderLog where F_ZWLF_TRANFERINSTATE='0'  and  FID in ('{0}')", ids);
                    ...
                    if (dt_L.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt_L.Rows.Count; i++)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    msg = zWLF_GetTransferIn.GetTransferIn(parameters, context);
                                }
                                else
                                {
                                ...
                                sql = string.Format(@"/*dialect*/ update ZWLF_T_OrderLog set F_ZWLF_NOTE='{1}'  where F_ZWLF_ID='{0}';", F_ZWLF_ID, mssg);
                    ...
                    {
                        this.View.ShowErrMessage("请选择胜途单据类型为胜途调拨入库，并且生成金蝶其他入库状态是失败的数据行！");
                        return;
                    }
                }
                else
