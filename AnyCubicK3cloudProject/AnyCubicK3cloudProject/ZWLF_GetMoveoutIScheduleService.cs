...
{
    [Description("获取胜途所有海外仓之间的调拨出库单")]
    [Kingdee.BOS.Util.HotUpdate]
    public class ZWLF_GetMoveoutIScheduleService : IScheduleService
    {
        /// <summary>
        /// 定时任务
        /// </summary>
        ...
        public void Run(Kingdee.BOS.Context ctx, Schedule schedule)
        {
            try
            {
                ZWLF_GetMoveout zWLF_HKGetSale = new ZWLF_GetMoveout();
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        msg = zWLF_HKGetSale.GetMoveout(parameters, context);
                        if (msg.status)
                        {
